...
5. **One camera queue** — done Sep 19 2026, corrected Sep 20. `resolveDriveCameraCommand` (`web/src/ui/driveCameraQueue.ts`) is the single per-frame decision: drone > resync > follow. Jeff, the chrome-settle snap, and reclaim publish intents; only the rAF loop writes to Mapbox. This replaced the many-writer design that produced the 435 dump. **A radio hold degrades the writer, it does not stop the camera** — 438 froze for 125 s of a 240 s drive and the puck left the screen. Hard follow needs no tiles, so we keep following over bare background. Hard follow places the puck on the yard line by writing the pose, then solving for the center in **ground space** (`centerPuttingPuckAtScreenPoint` — the solve Mapbox's own `setLocationAtPoint` does). Never reflect screen pixels and never pre-compute the shift: at pitch 68 pixels are not proportional to metres, which overshot the correction ~2x and kept the puck off the yard line, and a 439 pre-shift double-counted landscape padding and threw the puck a half block left on Go. The pan path never had either bug because `easeTo({ padding, offset })` calls Mapbox's solver.
