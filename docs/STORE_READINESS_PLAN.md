...
- [x] **`PrivacyInfo.xcprivacy`** — App manifest at `web/ios/App/App/PrivacyInfo.xcprivacy`. Tracking **on** (ATT + AdMob personalized ads when authorized): Google ad domains listed, Device ID + Advertising Data with `tracking: true`, location/search/crash still `tracking: false`. AdMob’s SDK ships its own manifest alongside this file.
- [x] **Drop `armv7`** — `UIRequiredDeviceCapabilities` is `arm64`.
- [x] **Real AdMob app ID** — `GADApplicationIdentifier` and `VITE_ADMOB_BANNER_UNIT_ID` use the StormPath AdMob app/unit (not Google’s sample ids). `.env.production` keeps test mode **off**.
...
- [x] **Verify export compliance** — `ITSAppUsesNonExemptEncryption=false` in plist + CI export options. Already correct; just keep an eye on it if any new SDK is added.
