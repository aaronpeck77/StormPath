...
That end state implies a **closed loop**: live reflectivity → cost field on the road graph → shortest-path refresh at high frequency → validation against new frames. Consumer routers (Mapbox Directions, etc.) **do not** implement that loop; they optimize distance/time/traffic rules only.
...
| **Routing** | Trip lines come from **Mapbox Directions**; they **do not** read radar tiles. There is **no** continuous reroute from storm motion. |
